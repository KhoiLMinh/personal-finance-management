...
class BudgetServiceImplTest {
    ...
    void createBudget_DuplicateBudget_ThrowsException() {
        ...
        RuntimeException exception = assertThrows(RuntimeException.class, () -> {
            budgetService.createBudget(1L, request);
        });
    ...
    void updateBudget_ValidRequest_Success() {
        ...
        assertNotNull(result);
        assertEquals(6000000.0, result.getLimitAmount());
        assertEquals(6000000.0, mockBudget.getLimitAmount()); // Entity thực tế bị thay đổi
    ...
    void getBudgetById_NotOwner_ThrowsException() {
        ...
        when(budgetRepository.findByIdAndUserId(budgetId, hackerId)).thenReturn(Optional.empty());

        RuntimeException exception = assertThrows(RuntimeException.class, () -> {
            budgetService.getBudgetById(budgetId, hackerId);
        });
