...
export default function TransactionFilter({ filterType, setFilterType, search, setSearch }: Props) {
  const types = [
        ...
        {types.map((t) => (
          <button
            ...
            className={`btn rounded-pill fw-medium border-0 px-4 py-2 ${
              filterType === t.value ? 'btn-info text-white' : 'btn-light text-muted bg-transparent'
            }`}
            style={filterType === t.value ? { backgroundColor: 'var(--color-primary)' } : {}}
          >
            {t.label}
          </button>
        ))}
