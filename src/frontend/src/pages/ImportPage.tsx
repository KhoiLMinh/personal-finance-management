...
import { UploadCloud, FileSpreadsheet, CheckCircle2, AlertTriangle, Bot } from 'lucide-react';

import walletService from '../services/walletService';
import { importService } from '../services/aiService'; // Import service đã cấu hình sẵn
...
export default function ImportPage() {
  ...
  const fetchWallets = async () => {
    try {
      ...
      setWallets(res);
      if (res.length > 0) {
        setSelectedWalletId(res[0].id.toString());
      }
    } catch (error) {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult(response);
      setSelectedFile(null); // Clear file sau khi thành công
    } catch (error: any) {
      console.error(error);
    ...
    <div className="p-4 flex-grow-1" style={{ backgroundColor: '#e2e8f0' }}>
      ...
      <Card className="border-0 rounded-4 mb-4 shadow-sm" style={{ backgroundColor: '#b0bec5' }}>
        <Card.Body className="p-4 d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
          <div>
            <h3 className="fw-bold text-dark mb-1">Nhập sao kê ngân hàng</h3>
            <p className="text-dark mb-0 opacity-75">Tự động hóa việc ghi chép chi tiêu với sự hỗ trợ của Trợ lý AI</p>
          </div>
      ...
      <Row className="g-4">
            ...
            <Card.Body className="p-4 p-md-5">
              ...
              <Form onSubmit={handleUpload}>
                ...
                <Form.Group className="mb-4">
                  ...
                  {/* Khu vực Upload làm nổi bật bằng CSS Dashed Border */}
                  <div className="position-relative p-5 text-center bg-light rounded-4" style={{ border: '2px dashed #cbd5e1' }}>
                    ...
                    {/* Input file ẩn đè lên toàn bộ khối div */}
                    <Form.Control 
                      type="file" 
                      accept=".csv"
        ...
        <Col md={5} lg={4}>
          <div className="d-flex flex-column gap-4 h-100">
            ...
            {/* THẺ HIỂN THỊ KẾT QUẢ (Chỉ hiện khi upload thành công) */}
            {result && (
              <Card className="border-0 rounded-4 shadow-sm bg-success text-white" style={{ background: 'linear-gradient(135deg, #059669 0%, #16a34a 100%)' }}>
                <Card.Body className="p-4">
                  <div className="d-flex align-items-center mb-3">
                    <CheckCircle2 size={24} className="me-2" />
                    <h5 className="fw-bold mb-0">Import Thành Công!</h5>
                  </div>
